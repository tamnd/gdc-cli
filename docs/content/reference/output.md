...
describes what users get. Pick a format with `-o`, or let gdc choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
gdc <command> -o table   # aligned columns for reading
gdc <command> -o jsonl   # one JSON object per line, for piping
gdc <command> -o json    # a single JSON array
gdc <command> -o csv     # spreadsheet friendly
gdc <command> -o tsv     # tab-separated
gdc <command> -o url     # just the URL column
gdc <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
gdc <command> --fields id,title,url
```
...
```bash
gdc <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
gdc <command>            # a table, because this is a terminal
gdc <command> | wc -l    # JSONL, because this is a pipe
```
